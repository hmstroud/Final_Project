...
SoVI_neighborhood <- read.csv("./Data/CRB_Vul_Sum.csv")%>%
  as.tibble()   
SoVI_centract <- read.csv("./Data/Climate_Ready_Boston_Social_Vulnerability.csv") %>% 
  as.tibble()
...
SoVI_centract <- SoVI_centract %>% rename(Housing_units=HU100_RE,
                         Tract_pop_count=POP100_RE,
                         Lmd_Eng=LEP,
                         POC=POC2) %>% 
                    mutate(Ppl_per_house= Tract_pop_count/Housing_units, 
